...
class UserListViewController: UIViewController {
    ...
    func fetchData() {
        ...
        APIManager.getUserInfoList(token: ImgMasterUtil.userToken!, success: { (userInfos) in
...
extension UserListViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.cellLabel.text = ("\(indexPath.row). phone: \(self.dataSource[indexPath.row].username!), mail: \(self.dataSource[indexPath.row].email!)")
        return cell
    }
